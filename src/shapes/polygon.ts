    ...
    for (let i = 0; i < points.length; i++) {
      const a0x =
        (points[i % points.length].x +
          ...
          points[(i + 2) % points.length].x) /
        6
      const a1x =
        (-points[i % points.length].x + points[(i + 2) % points.length].x) / 2
      const a2x =
        (points[i % points.length].x -
      ...
      const a3x =
        (-points[i % points.length].x +
          ...
          points[(i + 3) % points.length].x) /
        6

      const a0y =
        (points[i % points.length].y +
          4 * points[(i + 1) % points.length].y +
          points[(i + 2) % points.length].y) /
        6
      const a1y =
        (-points[i % points.length].y + points[(i + 2) % points.length].y) / 2
      const a2y =
        (points[i % points.length].y -
