...
<template>
  <div class="sidebar">
    ...
    <div class="param">
      ...
      <input v-model="toggle3d" type="checkbox" />
    </div>
    ...
    <div class="param">
      ...
      <input
        ...
        class="bg-gray-50 border border-gray-300 rounded-lg w-full block"
        type="number"
        min="1"
      />
    </div>
    ...
    <div class="param">
      <button
        ...
        :disabled="
          !isShapeSelected ||
          points.length < 0 ||
          (points.length > 0 && points[0].length < 4)
        "
        @click="$emit('createSpline')"
      >
        Создать сплайн
      </button>
    </div>
    <div class="param">
      <button
        ...
        :disabled="
          points.length === 0 ||
          points.length < 2 ||
          (points.length > 1 && points[0].length !== points[1].length)
        "
        @click="$emit('morphing')"
      >
        Морфинг
