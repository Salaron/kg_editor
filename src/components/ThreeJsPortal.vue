...
function initThreeJs(shapeManager: ShapeManager) {
  ...
  const camera = new THREE.PerspectiveCamera(45, windowRef.innerWidth / windowRef.innerHeight, 100, 200000)
  ...
  function render() {
    ...
    if (shapeManager.drawingShape)
      shapeManager.drawingShape.draw3d(scene)
    renderer.render(scene, camera)
  }

  function onWindowResize() {
    if (!windowRef || !renderer)
      return
