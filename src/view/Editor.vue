...
const mainCanvasComponent = ref<InstanceType<typeof CanvasComponent> | null>(
  null
)

const statusBarComponent = ref<InstanceType<typeof StatusBar> | null>(null)
const propertiesComponent = ref<InstanceType<typeof PropertiesSidebar> | null>(
  null
)
const operationsDialogComponent = ref<InstanceType<
  typeof OperationsDialog
> | null>(null)
const fractalTreeDialogComponent = ref<InstanceType<
  typeof FractalTreeDialog
> | null>(null)
...
function createSpline() {
  ...
  const splinePolygon = new Polygon(
    new ShapeProperties(
      "#ff00ff",
      baseShape.properties.lineWidth,
      "#000000",
      "1"
    ),
    new Vector(0, 0)
...
async function openOperationsDialog() {
  if (
    !operationsDialogComponent.value ||
    !shapeManager ||
  ...
  await operationsDialogComponent.value.openOperationsDialog(
...
async function openFractalTreeDialog() {
  if (!fractalTreeDialogComponent.value) return
...
async function openMorphingDialog() {
  ...
  await morphingDialogComponent.value.openMorphingDialog(
    canvasElement.offsetWidth,
    height.value,
    shapesCopy[0],
    shapesCopy[1]
...
function toggle3d(state: boolean) {
  if (!threeJsPortalComponent.value || !shapeManager) return
...
async function saveConfig() {
  ...
  const writableFile = await filePicker.createWritable()
  await writableFile.write(JSON.stringify(shapeManager.shapes))
...
<template>
  ...
  <OperationsDialog
    ref="operationsDialogComponent"
    @operation-dialog-closed="operationDialogClosed"
  />
  ...
  <div class="mainContainer pl-2 pr-2">
    ...
    <div class="editor" :style="{ height: height + 'px' }">
      ...
      <PropertiesSidebar
        ref="propertiesComponent"
        @open-operations="openOperationsDialog"
        @open-fractal-tree="openFractalTreeDialog"
        @shape-properties-changed="shapePropertiesChanged"
