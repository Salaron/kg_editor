...
function drawTree(context: CanvasRenderingContext2D) {
  ...
  if (!translated) {
    context.translate(
      -shapeManager.canvasWidth / 2,
      -shapeManager.canvasHeight / 2
    )
    translated = true
  }
...
function init() {
  ...
  addBranch(
    ...
    Math.PI * 0.5,
    height.value * 0.9,
    parseInt(treeParams.value.deep)
  )
  adjustToScreen()
}
...
function addBranch(
  ...
  if (level > 0) {
    addBranch(
      x,
      y,
      direction + (parseInt(treeParams.value.angle.first) * Math.PI) / 180,
      length * parseFloat(treeParams.value.scale.first),
      level - 1
    )
    addBranch(
      x,
      y,
      direction +
        (parseInt(treeParams.value.angle.second) * -1 * Math.PI) / 180,
      length * parseFloat(treeParams.value.scale.second),
      level - 1
    )
  }
}
